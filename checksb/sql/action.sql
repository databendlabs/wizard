-- Query 1: Conditional Merge Based on Order Status
...
-- Query 2: Merging with Aggregated Data
...
-- Query 3: Merge with Date-Based Condition
...
-- Query 4: Random Swap of Buy and Sell Orders
...
-- Query 5: Merging with a Fixed Increase in Quantity
...
-- Query 6: Merge for Creating Duplicate Orders with New IDs
...
-- Query 7: Merge with Subquery Join
...
-- Query 8: Merge with Date Ranges and Status Change
...
-- Query 9: Complex Merge with Nested Subqueries
...
MERGE INTO orders USING (
    ...
    WHERE rn = 1
) AS latest_orders ON orders.order_id = latest_orders.order_id
    WHEN MATCHED THEN
        UPDATE SET orders.status = 'latest';

-- Query 12: Merge Based on Average Asset Value
